...
const HELP_FAQ = [
  {
    ...
    items: [
      ...
      ['Does Njia filter courses by my CBE pathway?', 'No, deliberately. Filtering would need a verified map from each pathway\'s subjects to every programme\'s required subjects, and that map is not published in a form worth trusting with your options. A confident filter built on a guess would quietly hide courses you can actually do — which is a worse failure than making you check. So Njia explains the constraint and leaves the catalogue open. Confirm subject requirements for any specific programme with the institution or KUCCPS. This will change if the mapping is ever published properly.'],
