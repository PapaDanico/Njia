...
const isInk = (name) => name !== '--pink' && (/ink$/.test(name) || /^--text-(primary|secondary|muted)$/.test(name));
...
const DIMMED = [
  {
    ...
    opacity: 0.7,
    effective: { light: 3.61, dark: 5.50 },
    floor: 3.0,
    why: 'aria-hidden disclosure glyph. Not text for 1.4.3, but it is the only '
       ...
       + 'Was 0.6 and composited to 2.89:1 in light mode.'
  },
  {
  ...
  },
  {
    ...
    opacity: 0.92,
    effective: { light: 12.59, dark: 13.36 },
    floor: 4.5,
...
test('the compositing model reproduces the bug it exists to catch', () => {
  ...
  assert.ok(Math.abs(contrast(secondary, card) - 7.02) < 0.05,
    'the undimmed ratio no longer matches the 7.02:1 measured in the browser');
  const dimmed = contrast(composite(secondary, card, 0.75), card);
  assert.ok(Math.abs(dimmed - 3.84) < 0.05,
    `the model computes ${dimmed.toFixed(2)}:1 where the browser measured 3.84:1 — `
